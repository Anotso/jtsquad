...
function edina_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		var telefone 	= jQuery(".contact_form #telefone").val();
		...
		if(name===''||email===''||telefone===''||message===''){
			...
			jQuery.post("modal/contact.php",{ ajax_name: name, ajax_email: email, ajax_telefone: telefone, ajax_message:message, ajax_subject: subject}, function(data) {
...
function edina_tm_animate_text(){
	...
	var animateSpan			= jQuery('.edina_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["DESENVOLVEDOR FULLSTACK", "ESPECIALISTA EM SEO E MARKETING DIGITAL", "ESPECIALISTA WORDPRESS"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
